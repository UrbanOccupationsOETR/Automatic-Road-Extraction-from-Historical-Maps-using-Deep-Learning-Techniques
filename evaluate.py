...
def evaluate(model, generator):
    ...
    with torch.no_grad():
        for idx in tqdm.tqdm(range(len(generator))):
            X,y = generator[idx]
            #X = X.detach().cpu().numpy() #no need to further complicate the computational burden.
...
def calculate_metrics(y_preds,ys):
    ...
    Calculate evaluation metrics for a given *y* and *y_hat* arrays. 
    """
    y_preds = np.asarray(y_preds)
    ys = np.asarray(ys)

    include_label = [1,2,3,4,5] # omit background during metric calculation 
